        ...
        <div class="layout-page">
            ...
            >
                ...
                <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
                    <!-- Search -->
                    <form action="<c:url value="/noticeboard/list"/>" method="get" id="search_frm">
            ...
            <div class="content-wrapper">
                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light"></span> 공지사항 게시판 </h4>
                    <hr class="my-1" />
                    ...
                    <div class="card">
                        ...
                        <div class="table-responsive text-nowrap">
                            <table class="table table-hover">
                                ...
                                <tr>
                                    <th style="min-width: 80px;">글번호</th>
                                    <th class="w-50" style="min-width: 300px;">제목</th>
                                    <th style="min-width: 80px;">작성자</th>
                                    <th style="min-width: 80px;">작성일</th>
                                    <th style="min-width: 66px;">조회</th>
                                </tr>
                                </thead>
                                ...
                                <tr id="notice">
                                    ...
                                    <td class="text-start"><a href="<c:url value="/noticeboard/read${ph.sc.queryString}&bno=${noticeBoardDto.bno}"/>"><c:out value='${noticeBoardDto.title}'/></a></td>
                                    <td>${noticeBoardDto.name}</td>
                                    <c:choose>
                                        <c:when test="${noticeBoardDto.reg_date.time >= startOfToday}">
                                            <td><fmt:formatDate value="${noticeBoardDto.reg_date}" pattern="HH:mm" type="time"/></td>
                                        </c:when>
                                        <c:otherwise>
                                            <td><fmt:formatDate value="${noticeBoardDto.reg_date}" pattern="yyyy-MM-dd" type="date"/></td>
                                        </c:otherwise>
                                    </c:choose>
                    ...
                    <div class="mt-3 w-100 position-relative">
                        <div class="position-absolute end-0">
                            <button class="btn btn-primary w-px-100" onclick="location.href='<c:url value="/noticeboard/write"/>'">글쓰기</button>
                        </div>
                        <div class="d-flex justify-content-center d-grid gap-2">
                            <c:if test="${totalCnt!=null && totalCnt!=0}">
                                <c:if test="${ph.showPrev}">
                                    <a class="page" href="<c:url value="/noticeboard/list${ph.sc.getQueryString(ph.beginPage-1)}"/>">&lt;</a>
                                </c:if>
                                <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
                                    <a class="page ${i==ph.sc.page? "paging-active" : ""}" href="<c:url value="/noticeboard/list${ph.sc.getQueryString(i)}"/>">${i}</a>
                                </c:forEach>
                                <c:if test="${ph.showNext}">
                                    <a class="page" href="<c:url value="/noticeboard/list${ph.sc.getQueryString(ph.endPage+1)}"/>">&gt;</a>
                                </c:if>
                            </c:if>
                        </div>
                    </div>
